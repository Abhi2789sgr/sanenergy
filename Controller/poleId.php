...
foreach ($devData as $dRow){
	...
	if(isset($projectArr[$dRow['project']])){
		...
		$projectFirstTwo = substr($projeName, 0, 2);
	}
	$distName = " ";
	if(isset($distArr[$dRow['district']])){
		$distName = $distArr[$dRow['district']];
		$districtFirstTwo =substr($distName, 0, 2);
	}
	$blockName = " ";
	if(isset($blockArr[$dRow['block']])){
		$blockName = $blockArr[$dRow['block']];
		$blockFirstTwo = substr($blockName, 0, 2);
	}
	$panchName = " ";
	if(isset($panchArr[$dRow['panchayat']])){
		$panchName = $panchArr[$dRow['panchayat']];
		$panchFirstTwo = substr($panchName, 0, 2);
	}
	$wardName = " ";
	if(isset($wardArr[$dRow['ward']])){
		$wardName = $wardArr[$dRow['ward']];
		$wardFirstTwo = substr($wardName, 0, 2);
	}

    // Concatenate the first two words of each variable into a string
    $newPoleId = $dRow['name'].'/'.$projectFirstTwo .'/'. $districtFirstTwo .'/'. $blockFirstTwo .'/'. $panchFirstTwo .'/'. $wardFirstTwo;

	$dateTimeArr = explode(" ", $dRow["time"]);

	$csvData = $dRow['dev_id'].",".$newPoleId.",".$projeName.",".$distName.",".$blockName.",".$panchName.",".$wardName.",".$dRow['luminary_qr'].",".$dRow['battery_qr'].",".$dRow['panel_qr'].",".$dRow['lat'].",".$dRow['lng'].",".$dRow['remark'].",".$dRow['updated_by'].",".$dateTimeArr[0].",".$dateTimeArr[1]."
".$csvData;
