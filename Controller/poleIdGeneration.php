...
foreach ($devData as $dRow) {
	...
	$originalString = explode("/" , $dRow['name']);
	$arraySize = count($originalString);
	$poleIdName = end($originalString);

	$newString = $projectFirstTwo .'/'. $districtFirstTwo .'/'. $blockFirstTwo .'/'. $panchFirstTwo .'/'. $wardName .'/'.$poleIdName;
        
    if($dRow['name'] != $newString){
    	echo "<br>";
    	echo $dRow['dev_id']." pole id updated to ".$newString;
