...
if($bypass)
{
  ...
  if ($conn->query($sql) === TRUE){
    $selectLatestData = "SELECT id FROM _g_data_latest WHERE device = '".$id."'";
    $latestDataResult = $conn->query($selectLatestData);
    if($latestDataResult && $latestDataResult->num_rows > 0){
      $time = date("Y-m-d H:i:s", time());
      $sqlLatest = "UPDATE _g_data_latest SET `v1`='".$q[1]."', `v2`='".$q[2]."', `v3`='".$q[3]."', `v4`='".$q[4]."', `v5`='".$q[5]."', `v6`='".$q[6]."', `v7`='".$q[7]."', `v8`='".$q[8]."', `v9`='".$q[9]."', `v10`='".$q[10]."', `v11`='".$q[11]."', `v12`='".$q[12]."', `v13`='".$q[13]."', `v14`='".$q[14]."', `v15`='".$q[15]."', `v16`='".$q[16]."', `v17`='".$q[17]."', `v18`='".$q[18]."', `v19`='".$q[19]."', `v20`='".$q[20]."',`time`='".$time."' WHERE device = '".$id."'";
    } else {
    ...
    if($changeByte[7] == '1'){
      if(intval($faultData[7]) == 1){
        ...
        $updateLightStatusQuery = "UPDATE `light_on_off` SET `off_status` = 1, `off_time` = '".$currDateTime."' WHERE device_imei = '".$id."' ORDER BY id DESC LIMIT 1";
