...
<input class="form-control" type="text" oninput="inventorySearchLocal(this.value)" placeholder="Search for IMEI.." style="margin:10px; width:250px;">
<div class="w3-quarter" style="overflow-y: scroll;height:346px;overflow-x:hidden;margin-left:10px;" id="InvData">
    
    <div class="w3-bar w3-card w3-button m8-fancy" style="margin-bottom:10px; background-color:#2196F3" w3-repeat="result">
...
<div class="" style="display:none; float: right; width:72%;" id="txtHint">
    <div class="w3-container">
        <div id="txtResult">
            <div class="w3-card w3-white w3-margin-bottom w3-row m8-fancy" style="border: 3px solid #2196F3;height:346px;">
...
<script>
    ...
    function inventorySearch(search = '') {
        ...
        timerId = setTimeout(function() {
            ...
            xhr.onload = function() {
                ...
                totalDevices = JSON.parse(result);
    ...
    function inventorySearchLocal(event){
        if(event.trim() != ""){
            clearTimeout(timerId);
            timerId = setTimeout(function(){
                var devices = totalDevices.result;
                var tempArr = []
                devices.forEach((dev, index)=>{
                    if(dev.deviceImei.indexOf(event) >= 0){
                        tempArr.push(dev);
                    }
                });

                w3.displayObject("InvData",{'result':tempArr});
            },1000);
        }else{
            w3.displayObject("InvData",totalDevices);
        }
    }
