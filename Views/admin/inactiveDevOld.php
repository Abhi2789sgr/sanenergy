<div class="w3-container w3-responsive">
	<table id="id01" class="w3-table-all">
		...
		<tr w3-repeat="result">
			...
			<td>{{Block}}</td>
			<td>{{District}}</td>
			<td><img src="{{FilePath}}" style="width:200px;"></td>
			<td><a href="https://www.google.com/maps/search/?api=1&query={{Lat}},{{Lng}}" target="_blank" class="w3-large"><i class="fa fa-globe w3-text-color"></i> Locate</a></a></td>
			<td>{{Remark}}</td>
			<td>
				<button class="w3-button w3-teal m8-fancy" style="width:100px;margin-right:10px;margin-bottom:4px;" onclick="authoriseDevice('{{Dev_id}}')">Accept</button>
			...
			<form id="data" enctype="multipart/form-data" onsubmit="submitForm(event)">
				...
				<div class="row modal-body mx-3 d-flex justify-content-center">
					<div class="md-form mb-5 col-auto" style="width:200px; ">
						...
						<input type="text" id="deviceName" name="deviceName" class="form-control validate" placeholder="Pole ID" pattern=".{0}|[a-z0-9]{3,3}\/[a-z0-9]{3,3}\/[a-z0-9]{3,3}\/[a-z0-9]{3,3}\/[a-z0-9]{3,3}\/[0-9]{3,3}" title="Either 0 OR 23 chars pro/dis/blo/pan/war/number (number should be of 3 digits)">
					</div>
...
<div id="maxEle" style="display: none;">
	<?php
	$sql = "SELECT COUNT(*) as totalItem FROM _f_device WHERE active=0";
...
<script>
	InactiveDevices.classList.add("active");
	ActiveDevices.classList.remove("active");
	//table
	active = 0;
	var logPageNo = 0;
