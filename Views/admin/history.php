...
<div class="w3-container w3-margin-bottom w3-row">
  ...
  <div class="w3-button w3-color w3-right w3-margin m8-fancy" id="setDefault" onclick="setDefault()" style="display:none;">Set Default</div>
		...
		<div w3-repeat="result" class="w3-quarter w3-mobile" style="padding:5px;">
		  <div class="w3-bar w3-card w3-button m8-fancy">
			<div class="" style="text-align:left!important;" onclick="goHistory('{{Name}}','{{ID}}','{{Branch}}')">
			  <span class="w3-medium" style="word-wrap: break-word; white-space: normal;">{{Name}}</span><br>
			  <span><small><label class="tot_devices">Total Devices : </label> {{DeviceCount}}</small></span>
			</div>
			<button class="w3-right m8-fancy w3-button" style="margin:-27px;"><i onclick="editName('{{Name}}','{{ID}}','{{Branch}}')" class="fa fa-edit"></i></button>
			<!-- <button class="w3-right m8-fancy w3-button" style="color:red;margin:-27px;margin-top: -63px;"><i onclick="deleteName('{{Name}}','{{ID}}','{{Branch}}')" class="fa fa-trash"></i></button> -->
		  </div>
		</div>
	  </div>
	  <div class="w3-threequarter" style="display:none;" id="id03">
		...
		<div class="w3-container">
			<div class="w3-card w3-white w3-margin-bottom w3-row m8-fancy">
				<div class="w3-color w3-center w3-padding">
					<div class="w3-row">
						<div class="w3-col s3 m3 l3">
							<div><b>Pole ID</b></div>
						</div>
						<div class="w3-col s3 m3 l3">
							<div><b>Luminary IMEI</b></div>
						</div>
						<div class="w3-col s3 m3 l3">
							<div><b>SIM No.</b></div>
						</div>
						<div class="w3-col s3 m3 l3">
							<div><b>Battery S.No.</b></div>
						</div>
					</div>
					<div class="w3-row">
						<div class="w3-col s3 m3 l3" style="word-wrap: break-word;" >
						...
						<div class="w3-col s3 m3 l3" style="word-wrap: break-word;">
							<div id="battery_qr">--</div>
						</div>
					</div>
					<div class="w3-row w3-margin-top">
						<div class="w3-col s4 m4 l4">
							<div><b>Panel S.No.</b></div>
						</div>
						<div class="w3-col s4 m4 l4">
							<div><b>Installation Date</b></div>
						</div>
						<div class="w3-col s4 m4 l4">
							<div><b>Last Reporting Date</b></div>
						</div>
					</div>
					<div class="w3-row">
						<div class="w3-col s4 m4 l4">
			...
			<div class="w3-row">
				...
				<div class="w3-col s6 m4 l4" style="padding:5px;"><button class="w3-button w3-color m8-fancy" style="width:100%;" onclick='window.open("/Controller/download.php?q="+w3.id("log_imei").innerHTML,"_blank");'><i class="fa fa-download"></i> Download Log</button></div>
				<div class="w3-col s6 m4 l4" style="padding:5px;"><button class="w3-button w3-color m8-fancy" style="width:100%;" onclick='window.open("/Controller/getOnOff.php?q="+w3.id("log_imei").innerHTML,"_blank");'><i class="fa fa-download"></i> ON/OFF Report</button></div>
  ...
  <div id="idDeleteDevice" class="w3-modal">
    <div class="w3-modal-content w3-animate-top w3-card-4" style="width:400px;height:auto;height:200px;">
      <header class="w3-container w3-color"> 
        <span onclick="document.getElementById('idDeleteDevice').style.display='none'" 
        class="w3-button w3-display-topright">&times;</span>
...
function setDefault()
{
	var idsd = document.getElementById("setDefault");
	var param = "id="+id_global+"&default=";
	if(idsd.innerHTML=="Set Default")
	{
		param+="1";
		idsd.innerHTML = "Unset Default";
	}
	else
	{
		param+="0";
		idsd.innerHTML = "Set Default";
	}
	ajax("../Controller/admin/setDefault.php",param);
}
...
function setDetails(myObject)
{
	...
	// console.log("extraDetails : ",myObject.result);
...
function goHistory(name, id, branch)
{
	...
	var idsd = document.getElementById("setDefault");
	if(name.split(" ")[0]=="(4000)")
		idsd.innerHTML="Unset Default";
	else
		idsd.innerHTML="Set Default";
	if(branch=="6"){
		idsd.style.display="block";
		document.getElementById("exportReport").style.display="block";
	}else{
		idsd.style.display="none";
	}
...
function get2(r) {
	...
	w3.id("sim_no").innerHTML= r.sim_no!=undefined && r.sim_no!=undefined != null && r.sim_no!='' ? r.sim_no : "--";
	try{		
...
function get2_bak(url) {
	...
	xhttp.onload = function() {
		...
		try {
			...
			const cleanBatteryData = 0;
			if(batteryfaultDT.length >= 10){
				cleanBatteryData= parseInt(batteryfaultDT.replace(/[\s-:]/g, ''));
			}
			const panelfaultDT = r.PfaultDT;
			const cleanPanelData = 0;
			if(panelfaultDT >= 0){
				cleanPanelData = parseInt(panelfaultDT.replace(/[\s-:]/g, ''));
			}
			const lumifaultDT = r.LfaultDT;
			const cleanlumiData = 0;
			if(lumifaultDT >= 0){
				cleanlumiData = parseInt(lumifaultDT.replace(/[\s-:]/g, ''));
			}
			...
			const cleanBatteryResolve = 0;
			if(batteryResolve.length >= 10){
				cleanBatteryResolve = parseInt(batteryResolve.replace(/[\s-:]/g, ''));		
			}

			const panelResolve = r.BfaultR;
			const cleanPanelResolve = 0;
			if(panelResolve.length >= 10){
				cleanPanelResolve = parseInt(panelResolve.replace(/[\s-:]/g, ''));
			}	

			const luminaryresolve = r.BfaultR;
			const cleanLuminaryResolve = 0;
			if(luminaryresolve >= 10){
				cleanLuminaryResolve = parseInt(luminaryresolve.replace(/[\s-:]/g, ''));		
			}
...
function editName(name , id , parent){
		...
		w3.id("editDeviceButton").innerHTML = '<button class="w3-color w3-button w3-section w3-col s12 m12 l12 m8-fancy" onclick="updateDname('+ id +','+ parent +')">Save</button>';
	}
	
	function updateDname(id , parent){
		var value = document.getElementById('dName').value;
		var param = "name="+value+ "&id=" + id + "&branch=" +parent;
		if(parseInt(parent) > 9999){
			param = "name="+value+ "&id=" + id + "&branch=" +1;
		}
