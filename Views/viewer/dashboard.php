...
<div class="w3-container w3-responsive tableFixHead w3-small m8-fancy" id="container_id01" >
...
<script>
	...
	function getTableData(proj = parseInt(project), dist = "", bloc = "", panc = "", ward = "", imei = "", poleID = "", filter = 1) {
		//filter = 0 means no filter
		//filter = 1 means project hierarchy
		//filter = 2 means imei search;
		//filter = 3 means poleId search
		let url = `&project_id=<?php echo $branch_value; ?>&district_id=${dist}&block_id=${bloc}&panchayat_id=${panc}&&ward_id=${ward}&imei=${imei}&pole_id=${poleID}`;
	...
	table_btn.addEventListener("click", (event) => {
		...
		console.log("Project: " + parseInt(project), "dist: " + district.value, "block: " + block.value, "panch: " + panchayat.value, "ward: " + ward.value)
